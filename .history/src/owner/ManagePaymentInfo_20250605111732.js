...
const ManagePaymentInfo = () => {
  ...
  const [loading, setLoading] = useState(true); // Fixed: Added missing semicolon here
        ...
        <main className="flex-grow-1 overflow-auto p-4" style={{ backgroundColor: "#f8f9fa" }}>
          <div className="container-fluid mt-4">
            <div className="card shadow-lg p-4 bg-white border-0 rounded-4">
              ...
              {!loading && !error && (
                <>
                  {/* Revenue Summary Cards - Adjusted font sizes for compactness */}
                  <div className="row g-3 mb-4"> {/* Reduced margin-bottom */}
                    <div className="col-md-6 col-lg-3 d-flex">
                      <div className="card text-center text-white bg-gradient-success flex-fill shadow-sm border-0 rounded-3 animate__animated animate__fadeInUp">
                        <div className="card-body p-3"> {/* Reduced padding */}
                          <i className="bi bi-graph-up-arrow fs-4 mb-1"></i> {/* Reduced icon size */}
                          <h6 className="card-title text-black">Total Revenue</h6> {/* Reduced heading size, changed to black */}
                          <p className="card-text fs-5 fw-bold text-black">{formatTsh(revenueSummary.totalRevenue)}</p> {/* Reduced font size */}
                        </div>
                      </div>
                    </div>
                    <div className="col-md-6 col-lg-3 d-flex">
                      <div className="card text-center text-white bg-gradient-primary flex-fill shadow-sm border-0 rounded-3 animate__animated animate__fadeInUp animate__delay-1s">
                        <div className="card-body p-3"> {/* Reduced padding */}
                          <i className="bi bi-cash-stack fs-4 mb-1"></i> {/* Reduced icon size */}
                          <h6 className="card-title text-black">Total Payouts</h6> {/* Reduced heading size, changed to black */}
                          <p className="card-text fs-5 fw-bold text-black">{formatTsh(revenueSummary.totalPayouts)}</p> {/* Reduced font size */}
                        </div>
                      </div>
                    </div>
                    <div className="col-md-6 col-lg-3 d-flex">
                      <div className="card text-center text-white bg-gradient-info flex-fill shadow-sm border-0 rounded-3 animate__animated animate__fadeInUp animate__delay-2s">
                        <div className="card-body p-3"> {/* Reduced padding */}
                          <i className="bi bi-wallet2 fs-4 mb-1"></i> {/* Reduced icon size */}
                          <h6 className="card-title text-black">Current Balance</h6> {/* Reduced heading size, changed to black */}
                          <p className="card-text fs-5 fw-bold text-black">{formatTsh(revenueSummary.balance)}</p> {/* Reduced font size */}
                        </div>
                      </div>
                    </div>
                    <div className="col-md-6 col-lg-3 d-flex">
                      <div className="card text-center text-dark bg-gradient-warning flex-fill shadow-sm border-0 rounded-3 animate__animated animate__fadeInUp animate__delay-3s">
                        <div className="card-body p-3"> {/* Reduced padding */}
                          <i className="bi bi-hourglass fs-4 mb-1"></i> {/* Reduced icon size */}
                          <h6 className="card-title text-opacity-75">Pending Payouts</h6> {/* Reduced heading size */}
                          <p className="card-text fs-5 fw-bold">{formatTsh(revenueSummary.pendingPayouts)}</p> {/* Reduced font size */}
                        </div>
                      </div>
                    </div>
                  </div>

                  {/* Removed hr for closer proximity */}
                  {/* <hr className="my-5 border-2 border-primary-subtle" /> */}

                  {/* Recent Transactions Table */}
                  <h4 className="mb-3 text-secondary fw-bold text-center"><i className="bi bi-receipt-cutoff me-2"></i>Recent Transactions</h4> {/* Centered and adjusted margin-bottom */}
                    ...
                    <div className="table-responsive bg-white rounded-3 shadow-sm p-3">
                      <table className="table table-hover table-borderless table-striped align-middle">
                        <thead className="bg-primary text-white">
                          <tr>
                            <th scope="col" className="py-2">Transaction ID</th> {/* Reduced padding */}
                        ...
                        <tbody>
                          {transactions.map(transaction => (
                            <tr key={transaction.id}>
                              ...
                              <td>
                                <span className={`badge ${transaction.type === 'Income' ? 'bg-success-subtle text-success' : 'bg-primary-subtle text-primary'} p-1 rounded-pill`}> {/* Reduced padding */}
                                  {transaction.type}
                                </span>
                              </td>
                              <td>
                                <span className={`badge ${transaction.status === 'Completed' ? 'bg-success-subtle text-success' : 'bg-warning-subtle text-warning'} p-1 rounded-pill`}> {/* Reduced padding */}
                                  {transaction.status}
                                </span>
                              </td>
                            </tr>
