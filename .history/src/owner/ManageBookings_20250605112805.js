...
const ManageBookings = () => {
  ...
  const [selectedBooking, setSelectedBooking] = useState(null); // Hivi ndivyo inavyopaswa kuwa
              ...
              <>
                ...
                {pendingBookings.length > 0 && (
                  <div className="mb-4"> {/* Reduced margin */}
                    ...
                    <div className="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-2"> {/* Reduced gutter (g-2) */}
                      {pendingBookings.map((booking) => (
                        <div className="col" key={booking.id}>
                          <div className="card h-100 shadow-sm border-warning border-2 bg-light"> {/* Reduced shadow, border thickness */}
                            <div className="card-body d-flex flex-column p-2"> {/* Reduced padding (p-2) */}
                              <h6 className="card-title text-primary fw-bold mb-1 fs-6">{booking.vendorName}</h6> {/* Using fs-6 for slightly smaller title */}
                              <p className="card-subtitle mb-1 text-muted small"> {/* small class for subtitle */}
                                <i className="bi bi-shop me-1"></i> {booking.marketName} - Stall {booking.stallNumber}
                              </p>
                              <p className="card-text mb-0 small"> {/* Using small class for card text */}
                              ...
                              <p className="card-text mb-2 small"> {/* Using small class for card text */}
                              ...
                              <div className="mt-auto d-flex justify-content-between align-items-center pt-2"> {/* Reduced padding-top */}
                                <span className={`badge ${getStatusBadgeClass(booking.status)} text-uppercase py-1 px-2`}> {/* text-uppercase added, padding reduced */}
                                  <small>{booking.status}</small> {/* small tag inside badge for even smaller text */}
                                </span>
                                <div>
                                  <button
                                    className="btn btn-success btn-sm me-1 p-1" // Reduced padding (p-1)
                                  ...
                                  <button
                                    className="btn btn-danger btn-sm p-1" // Reduced padding (p-1)
